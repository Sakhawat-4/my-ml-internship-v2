...
**Status: needs your actual run.** I generated all six versions above for real inside this conversation — that's genuine Claude output, not simulated. I can't submit a prompt to ChatGPT myself, so this section needs you to actually do it:

1. Copy the exact v6 prompt above.
2. Paste it into a fresh ChatGPT conversation (no prior context, so it's a fair comparison).
3. Save the raw output.
4. Send it to me here — I'll write the comparison with you, pointing at specifics (did it keep the lookup-table shape or revert to if/elif? did it actually do the step-by-step reasoning or skip straight to code? did it invent a 5th reason_code or action_label that wasn't specified? did it validate types?).

Don't let the comparison section say "both were fine" — the eval criteria explicitly flags that as a fail. Look for at least one real divergence in tone, structure, or a failure point (e.g., one model silently dropping the type validation, or reordering severity differently).
